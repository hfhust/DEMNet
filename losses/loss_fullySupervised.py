...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # Apply sigmoid to convert the logits into probabilities for binary classification
        ...
        # Convert predictions to binary values (0 or 1)  # Convert probabilities to binary predictions

        # Calculate the intersection and union
        intersection = (inputs * targets).sum()
        total = (inputs ** 2).sum() + (targets ** 2).sum()

        # Calculate the Dice coefficient
        dice_coeff = (2.0 * intersection + self.smooth) / (total + self.smooth)

        # The loss is 1 - Dice coefficient
        return 1 - dice_coeff
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs, labels=None):
        # 假设inputs是一个列表，每个元素是形状为[batch_size, 1, h, w]的张量
        # 将所有特征图展平并拼接成一个大张量
        labels = labels.squeeze(1).view(labels.size(0), -1)
        outputs = []
        for output in inputs:
            # 展平输出，去掉中间的维度1
            cls_output = output.squeeze(1)  # [batch_size, h, w]
            outputs.append(cls_output)

        # 拼接所有层的输出
        combined_output = torch.cat([output.view(output.size(0), -1) for output in outputs], dim=1)  # [batch_size, n_anchors_all]

        if labels is not None:
            return self.get_classification_loss(combined_output, labels)
        else:
            # 如果没有标签，我们可能是在进行推理
            return combined_output

    def get_classification_loss(self, cls_preds, labels):
        # 计算分类损失
        # 假设labels是形状为[batch_size, n_anchors_all]的张量，其中包含了每个锚点的真实类别索引
        loss_cls = self.ce_loss(cls_preds, labels).sum()
        return loss_cls / cls_preds.size(0)  # 平均到每个样本上
# Example usage  
# Assume pred is the model output and target is the binary ground truth  
# pred = torch.randn(1, 1, 256, 256, requires_grad=True)  # Model output before sigmoid  
# target = torch.empty(1, 1, 256, 256).random_(2).float()  # Binary ground truth  
...
class rou2(nn.Module):
    def __init__(self, TVLoss_weight=0.001, w_weight=0.5):
